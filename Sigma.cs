...
namespace SplatoonScriptsOfficial.Duties.Endwalker.The_Omega_Protocol
{
    public unsafe class Dynamis_Sigma : SplatoonScript
    {
        ...
        public override void OnSettingsDraw()
        {
            ...
            {
                ImGuiEx.Text($"Marker alignment, north to south (Omega-M is true north)");
                ...
                foreach(var x in Conf.DirectionsSpots)
                {
                    var z = x.Value;
                    ImGui.SetNextItemWidth(200f);
                    if (ImGuiEx.EnumCombo($"{x.Key}", ref z))
                    {
                        Conf.DirectionsSpots[x.Key] = z;
                    }
                }
            }
